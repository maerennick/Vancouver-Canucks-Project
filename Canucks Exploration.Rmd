...
##### Directionality of trends in power play success through time across teams
...
As predicted by the R^2 value, there is a lot of variation in the data that is not well described by the model. However, we do see a decline in power play success on average through time. 
...
pp_goals<- ggplot(pp_season, aes(season, goalsFor/games_played, group = 1)) +
  geom_line(aes(group = team, color = division), alpha = 2/3) + 
  geom_line(stat = 'summary', fun.y = 'mean', size = .9, color = 'red') +
  scale_color_brewer(palette = 'Blues', direction = -1) +
  labs(x = 'Season (Start Year)',
...
heatmap<- ggplot(canucks_cors_long, aes(x = Var1, y = Var2, fill = value)) +
  geom_raster() + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title = element_blank()
...
Can_performance<- ggplot(canucks_data_long, aes(factor(season), Val_Zscore)) +
  geom_path(aes(group = 1), color = rdgy[8]) +
  geom_point(aes(color = sig), size = 1.75) +
  scale_color_manual(values = c(rdgy[3], rdgy[10]), name = "Z-Score") +
  scale_y_continuous(breaks = c(-2, 0, 2), minor_breaks = NULL) +
...
pp_success<- ggplot(model_data, aes(x = season, y = goalsFor_per_game)) +
  geom_point(color = "blue1", alpha=.02, size=3) +
  geom_line(aes(y = fitted_values), color = "darkgreen", size=1.2) +
  labs(
    x = "Season",
    y = "Average Power Play Goals per Game"
  ) +
...
player_ranked<- kable(head(rankings)) %>%
    kable_styling(bootstrap_options = c('striped', 'hover', 'responsive', 'condensed'))
